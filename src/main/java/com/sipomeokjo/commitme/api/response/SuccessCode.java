package com.sipomeokjo.commitme.api.response;

import lombok.Getter;
import org.springframework.http.HttpStatus;

@Getter
public enum SuccessCode implements ResponseCode {
    OK(HttpStatus.OK, "SUCCESS", "요청이 성공적으로 처리되었습니다."),
    LOGIN_URL_ISSUED(HttpStatus.OK, "SUCCESS", "로그인 URL 발급에 성공했습니다."),
    LOGIN_SUCCESS(HttpStatus.OK, "SUCCESS", "로그인에 성공했습니다."),
    LOGOUT_SUCCESS(HttpStatus.OK, "SUCCESS", "로그아웃에 성공했습니다."),
    ACCESS_TOKEN_REISSUED(HttpStatus.OK, "SUCCESS", "액세스 토큰 재발급에 성공했습니다."),
    POSITION_LIST_FETCHED(HttpStatus.OK, "SUCCESS", "포지션 목록 조회에 성공했습니다."),
    USER_PROFILE_FETCHED(HttpStatus.OK, "SUCCESS", "내 정보 조회에 성공했습니다."),
    USER_PROFILE_UPDATED(HttpStatus.OK, "SUCCESS", "회원 정보가 수정되었습니다."),
    ONBOARDING_COMPLETED(HttpStatus.OK, "SUCCESS", "회원가입이 완료되었습니다."),
    USER_SETTINGS_FETCHED(HttpStatus.OK, "SUCCESS", "사용자 설정 조회에 성공했습니다."),
    USER_SETTINGS_UPDATED(HttpStatus.OK, "SUCCESS", "사용자 설정 수정에 성공했습니다."),
    USER_DEACTIVATED(HttpStatus.OK, "SUCCESS", "회원 탈퇴가 완료되었습니다."),
    RESUME_LIST_FETCHED(HttpStatus.OK, "SUCCESS", "이력서 목록 조회에 성공했습니다."),
    RESUME_FETCHED(HttpStatus.OK, "SUCCESS", "이력서 상세 조회에 성공했습니다."),
    RESUME_CREATED(HttpStatus.CREATED, "CREATED", "이력서가 생성되었습니다."),
    RESUME_VERSION_FETCHED(HttpStatus.OK, "SUCCESS", "이력서 버전 조회에 성공했습니다."),
    RESUME_NAME_UPDATED(HttpStatus.OK, "SUCCESS", "이력서 이름이 수정되었습니다."),
    RESUME_EDIT_REQUESTED(HttpStatus.OK, "SUCCESS", "이력서 수정 요청에 성공했습니다."),
    RESUME_VERSION_SAVED(HttpStatus.OK, "SUCCESS", "이력서 버전이 저장되었습니다."),
    RESUME_DELETED(HttpStatus.OK, "SUCCESS", "이력서가 삭제되었습니다."),
    RESUME_PROFILE_CREATED(HttpStatus.CREATED, "CREATED", "이력서 프로필이 생성되었습니다."),
    RESUME_PROFILE_UPDATED(HttpStatus.OK, "SUCCESS", "이력서 프로필이 수정되었습니다."),
    RESUME_PROFILE_FETCHED(HttpStatus.OK, "SUCCESS", "이력서 프로필 조회에 성공했습니다."),
    NOTIFICATION_LIST_FETCHED(HttpStatus.OK, "SUCCESS", "알림 목록 조회에 성공했습니다."),
    NOTIFICATION_BADGE_FETCHED(HttpStatus.OK, "SUCCESS", "알림 배지 조회에 성공했습니다."),
    NOTIFICATION_MARKED_AS_SEEN(HttpStatus.OK, "SUCCESS", "알림을 확인함으로 표시했습니다."),
    NOTIFICATION_MARKED_AS_READ(HttpStatus.OK, "SUCCESS", "알림을 읽음으로 표시했습니다."),
    GITHUB_REPOS_FETCHED(HttpStatus.OK, "SUCCESS", "GitHub 레포지토리 목록 조회에 성공했습니다."),
    STT_TRANSCRIBED(HttpStatus.OK, "SUCCESS", "음성 인식이 완료되었습니다."),
    CHATROOM_FETCHED(HttpStatus.OK, "SUCCESS", "채팅방 목록 조회에 성공했습니다."),
    CHAT_HISTORY_FETCHED(HttpStatus.OK, "SUCCESS", "채팅 내역 조회에 성공했습니다."),
    CHAT_MESSAGE_SENT(HttpStatus.OK, "SUCCESS", "채팅 전송에 성공했습니다."),
    INTERVIEW_LIST_FETCHED(HttpStatus.OK, "SUCCESS", "면접 목록 조회에 성공했습니다."),
    INTERVIEW_FETCHED(HttpStatus.OK, "SUCCESS", "면접 조회에 성공했습니다."),
    INTERVIEW_TYPES_FETCHED(HttpStatus.OK, "SUCCESS", "면접 종류 목록 조회에 성공했습니다."),
    INTERVIEW_NAME_UPDATED(HttpStatus.OK, "SUCCESS", "면접 제목 수정에 성공했습니다."),
    INTERVIEW_DELETED(HttpStatus.OK, "SUCCESS", "면접 삭제에 성공했습니다."),
    INTERVIEW_STARTED(HttpStatus.CREATED, "CREATED", "면접이 시작되었습니다."),
    INTERVIEW_ANSWER_SENT(HttpStatus.OK, "SUCCESS", "답변이 전송되었습니다."),
    INTERVIEW_ENDED(HttpStatus.OK, "SUCCESS", "면접이 종료되었습니다."),
    INTERVIEW_CALLBACK_RECEIVED(HttpStatus.OK, "SUCCESS", "콜백이 정상 처리되었습니다."),
    UPLOAD_URL_ISSUED(HttpStatus.CREATED, "CREATED", "업로드 URL 발급에 성공했습니다."),
    UPLOAD_CONFIRMED(HttpStatus.OK, "SUCCESS", "업로드 완료 확정에 성공했습니다."),
    CREATED(HttpStatus.CREATED, "CREATED", "생성에 성공했습니다."),
    NO_CONTENT(HttpStatus.NO_CONTENT, "NO_CONTENT", "성공적으로 처리되었으며, 반환할 데이터가 없습니다.");

    private final HttpStatus httpStatus;
    private final String code;
    private final String message;

    SuccessCode(HttpStatus httpStatus, String code, String message) {
        this.httpStatus = httpStatus;
        this.code = code;
        this.message = message;
    }
}
